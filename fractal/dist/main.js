document.addEventListener("DOMContentLoaded", function() {
    ...
    if (elements.length === 0) console.error('No elements with the class "child_el" were found');
        ...
        div.addEventListener("mouseleave", function() {
            const parentSection = this.closest("section");
            parentSection.style.backgroundImage = "url('slide1.png')"; // Reset to the default image
        });
    });
});
document.addEventListener("DOMContentLoaded", function() {
    const slides = document.querySelectorAll(".slides > div");
    const image1 = document.querySelector(".slide-img-1"); // Select the first image
    const image2 = document.querySelector(".slide-img-2"); // Select the second image
    let currentSlide = 0;
